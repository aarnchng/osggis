...
osg::Image*
ImageUtils::resizeImage( osg::Image* input, unsigned int new_s, unsigned int new_t )
{
...
osg::Image*
ImageUtils::getFirstImage( osg::StateSet* ss )
{
...
void
ImageUtils::setFirstImage( osg::StateSet* ss, osg::Image* image )
{
    if ( ss && image )
    {
        for( unsigned int i=0; i<ss->getTextureAttributeList().size(); i++ )
